...
Django returns the the correct serializer data based on the url request from the Postman frontend. This way, only the data that is needed is provided in the API from Django.
...
class BookViewSet(viewsets.ModelViewSet):
    # default view set uses MiniBookSerializer to only send a small amount of data 
